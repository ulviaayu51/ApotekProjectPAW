...
class Customer extends CI_Controller {
  ...
  public function obat($kode_obat = NULL){
    ...
    if($kode_obat == NULL){
      ...
      $data['obat'] = $this->customer_model->getObat();

      $this->load->view('customer/index_view', $data);
    }
    else {
      $data['view_name'] = 'detail_obat';
      $data['obat'] = $this->customer_model->getObat($kode_obat);
  ...
  public function cek(){
    ...
    if($this->input->post('cek')){
      ...
      $cek_kode = $this->customer_model->checkKodeByKode($kode);

      if($cek_kode->num_rows() > 0){
        $detail = $this->customer_model->getInfoPemesanan($kode);
        ...
        $items = $this->customer_model->getDetailPemesanan($kode);
  ...
  public function konfirmasi(){
    ...
    if($this->input->post('konfirmasi')){
      ...
      $cek_kode = $this->customer_model->checkKode($kode_pesan, $identitas);

      if($cek_kode->num_rows() > 0){
        $pemesanan = $this->customer_model->getStatusPemesananByKode($kode_pesan, $identitas);
        $user_info = $this->customer_model->getUserInfo($pemesanan->id_pemesan);
        ...
        if($pemesanan->status == 'L'){
          ...
          if($this->customer_model->updateStatusPemesanan($kode_pesan))
